    ...
    window.WeAdminEdit = function (title, url, id, w, h) {
        ...
        layer.open({
            ...
            success: function (layero, index) {
                ...
                var area = []
                area[0] = order_data.receiveraddress.substr(0, 5)
                area[1] = (parserInt(district_code) / 1000) * 1000
                area[2] = (parserInt(district_code) / 100) * 100

                var getCity = getOrderDetail(area[0], 1)
                var getDistrict = getOrderDetail(area[1], 2)
                
                body.find('#province').val(getArea.province)
                for(i in getCity) {
                    body.find('#city').append('<option value="' + getCity[i].code + '">' + getCity[i].name + '</option>')
                }
                body.find('#city').val(order_data.city)
                for(i in getDistrict) {
                    body.find('#city').append('<option value="' + getDistrict[i].code + '">' + getDistrict[i].name + '</option>')
                }
                body.find('#district').val(order_data.district)
