...
def getProductsByOrderStatus(gte, lte):  # 想精确搜索就让 gte == lte
    ...
    for product in products:
        ...
        orders.append({
            ...
            "detail":
            util.MyServer +
            "product/tasks/byorder?order_id={}".format(order_id)
        })
    return orders
    ...
    if order_id != -1:
        ...
        if type(tasks) == type([]):
            ...
            if form.get("workshop_id") == None or form.get("amount") == None:
                raise Exception(util.ErrMsg[0].format(
                    "missing important field: workshop_id or amount"))
            ...
            raise Exception(util.ErrMsg[0].format(
                "need a list of tasks. create single task, please use 'createProduceTask'"
            ))
    else:
...
def updateTask(task_id, form):
    ...
    if type(form) == type({}):
        ...
        if form.get("material_getter"):
            task.material_getter = form.get("material_getter")
        if form.get("material_checker"):
            task.material_checker = form.get("material_checker")
        if form.get("material_distributon_date"):
            task.material_distributon_date = datetime.now().strftime(
                "%Y-%m-%d %H:%I:%S")
        task.save()
        return util.wrapTaskDTO(task)
    else:
        raise Exception(
            util.ErrMsg[0].format("updates info must in dict format"))
...
def deleteTasks(task_id_list):
    if (type(task_id_list) == type([])):
        ...
        raise Exception(
            util.ErrMsg[0].format("using deleteTask to delete single task."))
